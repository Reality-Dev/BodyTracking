...
public class FaceAnchor: Entity, HasFaceAnchoring {
    ...
    public internal(set) var face: FaceAnchorComponent {
        get {
            self.component(forType: FaceAnchorComponent.self) ?? .init()
        } set {
    ...
    public func attach(morphedEntity: FaceMorphedEntity,
        ...
        guard self.morphedEntities.contains(where: {$0 == morphedEntity}) == false else {
            print("Already added FaceMorphedEntity \(morphedEntity.name) to this FaceAnchor")
            return
        }
        
        self.morphedEntities.append(morphedEntity)
    ...
    public func destroy() {
      ...
      self.morphedEntities.removeAll()
