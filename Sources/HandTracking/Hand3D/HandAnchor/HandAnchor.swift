...
public class HandAnchor: Entity, HasHandAnchoring {
    ...
    public init(arView: ARView,
        ...
        self.handTracker2D = .init(arView: arView)
        ...
        self.handAnchorComponent = .init(depthBufferSelection: depthBufferSelection)
    ...
    public func attach(handTracker: HandTracker3D,
        ...
        guard self.handTrackers3D.contains(where: {$0 == handTracker}) == false else {
            print("Already added HandTracker3D \(handTracker.name) to this HandAnchor")
            return
        }
        self.handTrackers3D.append(handTracker)
